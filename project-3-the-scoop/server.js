...
try {
    const config = yaml.safeLoad(fs.readFileSync('scoopDatabase.yaml', 'utf8'));
...
function saveDatabase(){
  try {
    fs.writeFile(yaml.safeDump(database),'scoopDatabase.yaml' )
  }
  catch (e){
    console.log(e);
  }
...
function createComment(url, request){
  ...
  if(requestComment && requestComment.articleId && requestComment.username
    && requestComment.body
...
function deleteComment(url, request){
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const savedComment = database.comments[id];
  const savedArticle = savedComment && database.articles[savedComment.articleId];
  const savedUser = savedComment && database.users[savedComment.username];
  const articleComments = savedArticle && savedArticle.commentIds;
  const userComments = savedUser && savedUser.commentIds;
  const response = {};
